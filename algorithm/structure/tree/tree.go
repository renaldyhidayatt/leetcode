...
func searchTreeHelper[T constraints.Ordered](node, nilNode Node[T], key T) (Node[T], bool) {
	...
	if key < node.Key() {
		return searchTreeHelper[T](node.Left(), nilNode, key)
	}

	return searchTreeHelper[T](node.Right(), nilNode, key)
}
...
func postOrderRecursive[T constraints.Ordered](n, nilNode Node[T], traversal *[]T) {
	...
	postOrderRecursive[T](n.Left(), nilNode, traversal)
	postOrderRecursive[T](n.Right(), nilNode, traversal)
...
func predecessorHelper[T constraints.Ordered](node, nilNode Node[T]) (T, bool) {
	if node.Left() != nilNode {
		return maximum[T](node.Left(), nilNode).Key(), true
	}
