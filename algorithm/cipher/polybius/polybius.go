...
func NewPolybius(key string, size int, chars string) (*Polybius, error) {
	...
	return &Polybius{size: size, characters: chars, key: key}, nil
}
...
func (p *Polybius) decipher(chars []rune) (string, error) {
	...
	if row < 0 {
		return "", fmt.Errorf("%q does not exist in characters", chars[0])
	}

	col := strings.IndexRune(p.characters, chars[1])

	if col < 0 {
		return "", fmt.Errorf("%q does not exist in characters", chars[1])
	}
