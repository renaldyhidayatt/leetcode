...
func heapifyDown[T any](slice []T, N, i int, moreFunc func(i, j int) bool, swapFunc func(i, j int)) {
	...
	if max != 1 {
		swapFunc(i, max)
		heapifyDown[T](slice, N, max, moreFunc, swapFunc)
	}
...
func HeapSort[T constraints.Ordered](slice []T) []T {
	...
	for i := N/2 - 1; i >= 0; i-- {
		heapifyDown[T](slice, N, i, moreFunc, swapFunc)
	}

	for i := N - 1; i > 0; i-- {
		slice[0], slice[i] = slice[i], slice[0]
		heapifyDown[T](slice, i, 0, moreFunc, swapFunc)
	}
